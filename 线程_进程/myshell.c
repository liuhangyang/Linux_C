...
int find_command(char *);			//查找命令中可执行的程序：
...
void do_cmd(int argcount,char arglist[100][256])
{
	...
	int background = 0;	//表示命令中是否有后台运行的标识符&*/
	int status;
	int i,j;
	int fd;
	char* arg[argcount+1];
	...
	/*flag大于１,说明命令中含有多个>,<,|，符号，本程序不支持这样的命令，或者命令的格式不对，如"ls -l /tmp >"*/
	if(flag >1)
	{
		printf("Wrong command\n");
		return;
	}
	if(how == out_redirect)   //命令只含有一个输出重定项符号>*/
	{
		for(i=0;arg[i]!=NULL;i++)
		{
			if(strcmp(arg[i],">")==0)
	...
	if(how == have_pipe)	/*命令只含有一个管道符号|,把管道符号后门的部分存入argnext中，管道后面的部分是一个可执行的shell命令*/
	{
		for(i=0;arg[i]!=NULL;i++)
		{
			if(strcmp(arg[i],"|")==0)
	...
	switch(how)
	{
		case 0:
			...
			if(pid == 0)
				...
				/*函数说明：execvp()会从PATH幻镜变量所指的目录中查找符合参数file的文件名,找到后执行该文件,然后将argv的第二个参数传给欲执行的文件*/
				execvp(arg[0],arg);
				exit(0);
			}
			break;
